...
lv_color_t lv_img_buf_get_px_color(lv_img_dsc_t * dsc, lv_coord_t x, lv_coord_t y, const lv_style_t * style)
{
    ...
    uint32_t * buf_u32 = (uint32_t *)dsc->data;
    ...
    if(dsc->header.cf == LV_IMG_CF_TRUE_COLOR || dsc->header.cf == LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED) {
        ...
        if(rem == 0) {
            p_color.full = (buf_u32[mod] & 0xFFFF);
        } else {
            p_color.full = (buf_u32[mod] >> 16);
        }
    } else if(dsc->header.cf == LV_IMG_CF_TRUE_COLOR_ALPHA) {
        uint8_t px_size = lv_img_color_format_get_px_size(dsc->header.cf) >> 3;
        uint32_t px     = (dsc->header.w * y + x) * px_size;
        uint32_t mod = px / 4;
        uint32_t rem = px % 4;
        // memcpy(&p_color, &buf_u8[px], sizeof(lv_color_t));
        switch(rem) {
            case 0:
                ...
                break;
            case 1:
                p_color.full = (buf_u32[mod] & (0xFFFF << 8))>>8;
                break;
            case 2:
                p_color.full = (buf_u32[mod] & ((uint32_t)0xFFFF << 16))>>16;
                break;
            case 3:
                p_color.full = (buf_u32[mod] & ((uint32_t)0xFF << 24))>>24;
                p_color.full |= (buf_u32[mod + 1] & 0xFF )<<8;
                break;
...
lv_opa_t lv_img_buf_get_px_alpha(lv_img_dsc_t * dsc, lv_coord_t x, lv_coord_t y)
{
    ...
    if(dsc->header.cf == LV_IMG_CF_TRUE_COLOR_ALPHA) 
    {
        ...
        uint32_t mod = (px + LV_IMG_PX_SIZE_ALPHA_BYTE - 1) / 4;
        uint32_t rem = (px + LV_IMG_PX_SIZE_ALPHA_BYTE - 1) % 4;
        uint8_t temp;
        switch(rem) 
        {
        case 0:
            temp = buf_u32[mod] & 0xFF;
            break;
        case 1:
            temp = (buf_u32[mod] >> 8) & 0xFF;
            break;
        case 2:
            temp = (buf_u32[mod] >> 16) & 0xFF;
            break;
        case 3:
            temp = (buf_u32[mod] >> 24) & 0xFF;
            break;
...
void lv_img_buf_set_px_color(lv_img_dsc_t * dsc, lv_coord_t x, lv_coord_t y, lv_color_t c)
{
    ...
    if(dsc->header.cf == LV_IMG_CF_TRUE_COLOR || dsc->header.cf == LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED) {
        ...
        uint32_t px     = dsc->header.w * y + x;
        uint32_t mod = px / 2;
        uint32_t pos = (((px % 2) == 0) ? 0 : 16);
        // memcpy(&buf_u32[px], &c, px_size);
        buf_u32[mod] &= ~(0xFFFF << pos);
        buf_u32[mod] |= (c.full << pos);
    } else if(dsc->header.cf == LV_IMG_CF_TRUE_COLOR_ALPHA) {
        uint8_t px_size = lv_img_color_format_get_px_size(dsc->header.cf) >> 3;
        uint32_t px     = (dsc->header.w * y + x) * px_size;
        uint32_t mod = px / 4;
        ...
        switch(rem) {
            case 0:
                ...
                break;
            case 1:
                buf_u32[mod] &= ~(0xFFFF << 8);
                buf_u32[mod] |= c.full << 8;
                break;
            case 2:
                buf_u32[mod] &= ~((uint32_t)0xFFFF << 16);
                buf_u32[mod] |= c.full << 16;
                break;
            case 3:
                buf_u32[mod] &= ~((uint32_t)0xFF << 24);
                buf_u32[mod] |= (c.full & 0xFF) << 24;
                buf_u32[mod + 1] &= ~0xFF;
                buf_u32[mod + 1] |= c.full >> 8;
                break;
...
void lv_img_buf_set_px_alpha(lv_img_dsc_t * dsc, lv_coord_t x, lv_coord_t y, lv_opa_t opa)
{
    ...
    if(dsc->header.cf == LV_IMG_CF_TRUE_COLOR_ALPHA) {
        ...
        uint32_t mod = (px + px_size - 1) / 4;
        uint32_t rem = (px + px_size - 1) % 4;
